/**
 * useSubscriptions Hook
 * 管理订阅数据，支持缓存
 */

import { useEffect, useState } from "react"

import { fetchSubscriptions } from "~/lib/api/client"
import { getCacheData, setCacheData } from "~/lib/storage"
import type { Subscription } from "~/types"

const CACHE_KEY = "subscriptions_cache"
const CACHE_MAX_AGE = 5 * 60 * 1000 // 5分钟缓存

export function useSubscriptions(shouldFetch: boolean) {
  const [subscriptions, setSubscriptions] = useState<Subscription[]>([])
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)

  const loadSubscriptions = async (useCache = true) => {
    if (!shouldFetch) return

    // 先尝试从缓存加载
    if (useCache) {
      const cachedData = await getCacheData<Subscription[]>(
        CACHE_KEY,
        CACHE_MAX_AGE
      )
      if (cachedData) {
        console.log("[Subscriptions] 使用缓存数据")
        setSubscriptions(cachedData)
        setLoading(false)
        return
      }
    }

    setLoading(true)
    setError(null)

    try {
      const response = await fetchSubscriptions()

      if (response.success && response.data) {
        setSubscriptions(response.data)
        // 保存到缓存
        await setCacheData(CACHE_KEY, response.data)
      } else {
        setError(response.message || "获取订阅信息失败")
      }
    } catch (err) {
      setError(err instanceof Error ? err.message : "未知错误")
    } finally {
      setLoading(false)
    }
  }

  useEffect(() => {
    loadSubscriptions()
  }, [shouldFetch])

  return {
    subscriptions,
    loading,
    error,
    refresh: () => loadSubscriptions(false) // 刷新时不使用缓存
  }
}
